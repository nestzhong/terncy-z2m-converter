...
// Terncy/Xiaoyan TERNCY-WS10-D1, 1-gang wall switch.
//
// Derived from gateway firmware reverse engineering (node-struct):
//   modelID: TERNCY-WS10-D1
//   manufacturerName: Xiaoyan
//   manufacturerCode: 0x1228
//   private cluster: 0xfccc
//   endpoint 1: Basic, Identify, PollCtrl, 0xfccc (attrs 0x17-0x19, 0x1c, 0x1f-0x26, 0x28), no OnOff
//   endpoint 2: 0xfccc only
//   endpoint 3: 0xfccc only
//   endpoint 4: OnOff, 0xfccc  <- the only relay output
//
// The single relay output lives on endpoint 4, so it is exposed as l4.
// Endpoints 1-3 carry no OnOff server cluster; they still get action
// decoding because wireless button frames on 0xfccc can arrive from any
// endpoint.
//
...
// This model has not yet been re-verified on a physical WS10-D1.
...
const ENDPOINTS = {l1: 1, l2: 2, l3: 3, l4: 4};
// Endpoints carrying an OnOff server cluster (relay) per node-struct.
const RELAY_ENDPOINTS = {l4: 4};
...
function perRelayConfig(name, values, description) {
    return Object.keys(RELAY_ENDPOINTS).map((ep) =>
        exposes.enum(name, ea.STATE_SET, values).withEndpoint(ep)
            .withDescription(description).withCategory("config"),
    );
}

function perRelayBinary(name, description) {
    return Object.keys(RELAY_ENDPOINTS).map((ep) =>
...
export default {
    zigbeeModel: ["TERNCY-WS10-D1"],
    fingerprint: [{modelID: "TERNCY-WS10-D1", manufacturerName: "Xiaoyan"}],
    model: "TERNCY-WS10-D1",
    vendor: "Terncy",
    ...
    exposes: [
        ...Object.keys(RELAY_ENDPOINTS).map((ep) => e.switch().withEndpoint(ep)),
        ...perRelayConfig("operation_mode", ["control_relay", "wireless"], "Control relay or act as wireless switch"),
        ...perRelayBinary("relay_enabled", "Enable or disable relay while in wireless switch mode"),
        ...perRelayBinary("relay_constant_power", "Turn relay on, then decouple input for constant power"),
        ...perRelayConfig("wireless_led_status", Object.keys(WIRELESS_LED_STATUS), "LED state while in wireless switch mode"),
        ...perRelayConfig("led_feedback_mode", Object.keys(LED_FEEDBACK_MODE), "Relay-mode LED feedback relation"),
    ...
    configure: async (device, coordinatorEndpoint) => {
        for (const ID of Object.values(RELAY_ENDPOINTS)) {
            ...
            try {
                ...
                console.warn(`TERNCY-WS10-D1: skipped reporting setup for endpoint ${ID}: ${error.message}`);
            }
        }
    },
};
