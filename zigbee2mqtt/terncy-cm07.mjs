...
// Terncy/Xiaoyan TERNCY-CM07 curtain motor (sheer/dream curtain, with tilt).
//
...
//
// Same standard WindowCovering control as the CM01 (UpOpen/DownClose/Stop,
// GoToLiftPercentage = command 0x05). Additionally supports tilt through
// GoToTiltValue (command 0x07, u16 payload): zigbee-herdsman already numbers
// goToTiltValue 0x07, matching the gateway disassembly. The App-side slider
// for this command is an angle in degrees, -90..90; negative angles are sent
// as two's-complement u16 values (pending a sniff to confirm byte order).
// The device has no tilt position reporting channel verified yet, so
// tilt_angle is set-only. If the lift position direction is wrong on your
// unit, set the device option invert_cover: true.
//
...
const tzLocal = {
    terncyCurtainTiltAngle: {
        key: ["tilt_angle"],
        convertSet: async (entity, key, value, meta) => {
            const angle = Math.max(-90, Math.min(90, Math.round(Number(value))));
            const tiltvalue = angle < 0 ? 65536 + angle : angle;
...
export default {
    zigbeeModel: ["TERNCY-CM07"],
    fingerprint: [{modelID: "TERNCY-CM07", manufacturerName: "Xiaoyan"}],
    model: "TERNCY-CM07",
    vendor: "Terncy",
    description: "Curtain motor with tilt (sheer/dream curtain)",
    extend: [
    ...
    toZigbee: [
        ...
        tzLocal.terncyCurtainTiltAngle,
    ...
    exposes: [
        e.cover_position(),
        exposes.numeric("tilt_angle", ea.STATE_SET)
            .withUnit("°").withValueMin(-90).withValueMax(90)
            .withDescription("Venetian tilt angle; same -90..90 degree slider range as the Terncy app"),
    ...
    configure: async (device, coordinatorEndpoint) => {
        ...
        try {
            ...
            console.warn(`TERNCY-CM07: skipped reporting setup: ${error.message}`);
        }
    },
};
