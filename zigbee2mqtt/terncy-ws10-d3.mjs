...
// Terncy/Xiaoyan TERNCY-WS10-D3, 3-gang wall switch.
//
// Derived from gateway firmware reverse engineering (node-struct):
//   modelID: TERNCY-WS10-D3
//   manufacturerName: Xiaoyan
//   manufacturerCode: 0x1228
//   private cluster: 0xfccc
//   endpoint 1: Basic, Identify, OnOff, PollCtrl, 0xfccc (attrs 0x17-0x19, 0x1c, 0x1f-0x26, 0x28)
//   endpoint 2: OnOff, 0xfccc
//   endpoint 3: 0xfccc only (no OnOff server cluster; not exposed as a relay)
//   endpoint 4: OnOff, 0xfccc
//
// Relay outputs are therefore exposed for endpoints 1, 2 and 4. Endpoint 3
// still gets action decoding because wireless button frames on 0xfccc can
// arrive from any endpoint.
//
...
// This model has not yet been re-verified on a physical WS10-D3.
...
const RELAY_ENDPOINTS = {l1: 1, l2: 2, l4: 4};
...
export default {
    zigbeeModel: ["TERNCY-WS10-D3"],
    fingerprint: [{modelID: "TERNCY-WS10-D3", manufacturerName: "Xiaoyan"}],
    model: "TERNCY-WS10-D3",
    vendor: "Terncy",
    ...
    configure: async (device, coordinatorEndpoint) => {
        for (const ID of Object.values(RELAY_ENDPOINTS)) {
            ...
            try {
                ...
                console.warn(`TERNCY-WS10-D3: skipped reporting setup for endpoint ${ID}: ${error.message}`);
            }
        }
    },
};
